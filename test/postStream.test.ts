...
describe('reportPostStreamDiagnostics', () => {
  it('warns on raw <thinking> tags without separated reasoning', () => {
    const output = makeOutput();
    const progress = makeProgress();
    reportPostStreamDiagnostics(
      makeModel(),
      makeOptions(),
      outcome({ hadContent: true, sawRawThinkTags: true, finishReason: 'stop' }),
      0,
      progress,
      1,
      output,
    );
    expect(lines(output)).toContain('raw <thinking> tags detected in content');
    expect(progress.reports).toHaveLength(0);
  });

  it('warns when tool calls were truncated by token limit', () => {
    const output = makeOutput();
    const progress = makeProgress();
    reportPostStreamDiagnostics(
      makeModel(),
      makeOptions(),
      outcome({ hadToolCalls: true, finishReason: 'length' }),
      0,
      progress,
      1,
      output,
    );
    expect(lines(output)).toContain('tool call arguments may be truncated');
  });

  it('warns when Copilot required a tool call but model returned text only', () => {
    const output = makeOutput();
    const progress = makeProgress();
    reportPostStreamDiagnostics(
      makeModel(),
      makeOptions(vscode.LanguageModelChatToolMode.Required),
      outcome({ hadContent: true, finishReason: 'stop' }),
      0,
      progress,
      1,
      output,
    );
    expect(lines(output)).toContain('Copilot required a tool call but model returned text only');
  });

  it('surfaces an empty response with a user-visible note', () => {
    const output = makeOutput();
    const progress = makeProgress();
    reportPostStreamDiagnostics(
      makeModel(),
    ...
    expect(all).toContain('empty response');
    expect(all).toContain('finish_reason: stop');
    expect(progress.reports).toHaveLength(1);
    expect(String((progress.reports[0] as any).value)).toContain('The model returned no output');
  });

  it('warns and notes when content was truncated mid-stream', () => {
    const output = makeOutput();
    const progress = makeProgress();
    reportPostStreamDiagnostics(
      makeModel(),
      makeOptions(),
      outcome({ hadContent: true, finishReason: 'length' }),
      0,
      progress,
      1,
      output,
    );
    expect(lines(output)).toContain('response truncated at max output tokens');
    expect(progress.reports).toHaveLength(1);
    expect(String((progress.reports[0] as any).value)).toContain('Response truncated');
  });
});
