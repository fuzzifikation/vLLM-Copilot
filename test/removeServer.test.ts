...
/**
 * Tests for "Remove Server". The command is URL-addressed and URL-wide (§5):
 * every registry entry on the normalized URL is in play. With two credential
 * identities on one URL, first-match semantics would delete the WRONG entry —
 * these tests pin the multi-entry behavior.
 */
...
describe('removeServer command', () => {
  ...
  it('removes every unreferenced entry on the URL, leaving other URLs intact', async () => {
    const servers = [
      { id: 'srv-a', serverUrl: 'http://s:8000' },
      { id: 'srv-b', serverUrl: 'http://s:8000/' }, // same URL, trailing slash
    ...
    expect(updateCall![1].map((s: any) => s.id)).toEqual(['other']);
    expect(provider.clearCache).toHaveBeenCalled();
  });

  it('refuses when ANY entry on the URL is still referenced, naming the models', async () => {
    const servers = [
      { id: 'srv-a', serverUrl: 'http://s:8000' },
      { id: 'srv-b', serverUrl: 'http://s:8000' },
    ];
    const models = [{ id: 'm1', vllmModelId: 'm1', server: 'srv-b' }];
  ...
  it('does nothing when the URL has no entries', async () => {
