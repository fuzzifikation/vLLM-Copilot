...
describe('resolveServerConfig', () => {
  ...
  it('returns undefined when override is undefined', () => {
    expect(resolveServerConfig(undefined, servers)).toBeUndefined();
  });

  it('returns undefined when the registry is empty', () => {
    expect(resolveServerConfig(model, [])).toBeUndefined();
  });

  it('returns undefined when the model ref matches no entry (dangling ref)', () => {
    const dangling: ModelConfig = { id: 'test', server: 'ghost' };
    expect(resolveServerConfig(dangling, servers)).toBeUndefined();
  });
