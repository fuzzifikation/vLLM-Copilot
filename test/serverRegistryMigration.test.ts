...
describe('maybeRunServerRegistryMigration', () => {
  ...
  it('never logs header values — both dumps redact credentials, settings keep them', async () => {
    // The Output channel is user-visible and routinely pasted into bug
    // reports. Legacy inline headers (before) and the adopted registry entry
    // (after) must appear with values masked; the actual settings write keeps
    // the real credential — redaction is log-only.
    settings.models = [model({ id: 'a', requestHeaders: { Authorization: 'sk-CANARY-123' } })];

    await maybeRunServerRegistryMigration(context, output as never);

    const log = output.appendLine.mock.calls.map(c => String(c[0])).join('\n');
    expect(log).toContain('before:'); // dumps still happen — structure kept
    expect(log).not.toContain('sk-CANARY-123');
    expect(log).toContain('<redacted>');
    expect(log).toContain('Authorization'); // names survive, values do not
    expect((settings.servers as Array<Record<string, unknown>>)[0].requestHeaders)
      .toEqual({ Authorization: 'sk-CANARY-123' });
  });

  it('warns when the members of one connection disagree about the backend protocol', async () => {
    settings.models = [
      model({ id: 'gpusaur', serverType: 'ollama' }),
      model({ id: 'confused', serverType: 'vllm' }),
    ];
    ...
    expect(log).toContain('different backends');
    expect(log).toContain('"gpusaur"');
    expect(log).toContain('"confused"');
    expect(log).toContain('ollama');
    // The migration still lands — the conflict is reported, not fatal.
    expect(stateStore[FLAG]).toBe('done');
  });
});
