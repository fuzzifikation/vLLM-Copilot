...
Also, not every bugfix deserves its own version upgrade. Do not upgrade version after fixing, unless told by the user.
...
- **`dashboard.ts:208`** — **moved to False Positives (2026-07-28):** the line number was stale (the `Promise.all` is at `dashboard.ts:238`). More importantly, the entry claims "no per-fetch timeout, one slow server blocks the entire dashboard" — but each entry in that `Promise.all` calls `fetchServerMetrics`, which has its own internal 5s `AbortController` (`vllmMetrics.ts:252-302`). The per-fetch timeout IS present, just one layer down. (The shared-abort mid-flight timeout case was fixed in v1.20.1 — each inner catch now re-throws when the controller is aborted.)<tool_call>arg_value></tool_call>
...
- **`saveModelConfig` and `serverSettingsView.ts::saveModelConfig` write to Global only.** Filed because if `vllm-copilot.models` is set at workspace scope, the write is shadowed by the workspace value (VS Code precedence is Default < Global < Workspace). **Rejected as intentional:** the extension's design is "always write to global user settings"; the workspace-scope case is out of scope for now and would need an explicit design discussion. The now-removed `migration.ts` also had this characteristic but was dead code — the module was deleted in v1.20.0.
