...
### P2 - Auto-continue mutation of `openaiMessages` by index is fragile; retry can fire after a tool-call turn
...
- **Real bug — retry does not check `outcome.hadToolCalls`:** the retry condition at `provider.ts:313` is `(!outcome.hadContent || endsWithColon) && outcome.finishReason === 'stop' && attempt < maxRetries`. If the first attempt streams a tool call to Copilot and then stops (`finish_reason: 'stop'`) with no text content, the loop retries and pushes an assistant prefill into the next request *after* Copilot has already received a tool call. The originally-claimed "tool call arguments misinterpreted as plain text" is **false** — in the nudge branch `assistantPrefill` stays empty (`outcome.hadContent` is false), so no tool-call text is replayed. The real concern is the retry firing at all after a tool-call turn — the model already took an action. **Fix:** add `&& !outcome.hadToolCalls` to the retry condition.
