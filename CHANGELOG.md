...
- **Works in the new Agents window ("Open in Agents")** — the extension now auto-enables VS Code 1.135's experimental `chat.agentHost.byokModels.enabled` setting and its own `extensions.supportAgentsWindow` entry, so your vLLM models show up in Agent Host sessions (Copilot harness) in the dedicated Agents window. Values you set yourself in `settings.json` are never overwritten — an explicit `false` stays yours — and nothing is written on VS Code builds that don't know these settings. The Agent Host is a separate process: fully restart VS Code after the settings land.
- **Output-length picker** — models (and presets) whose `maxOutputTokens` is an **array** of response lengths now show a **second dropdown in the Copilot model picker**, right next to Model Mode, for the response length (e.g. `64K / 32K / 16K`). One field, two shapes: a plain number stays a plain cap, an array is the menu — first entry is the default and the desired budget. It is fully independent of modes: modes describe *behavior* (thinking, sampling), the new dropdown owns *length*. Entries above the model's ceiling are dropped from the menu, and your pick overrides any per-mode / `defaultParams` `max_tokens`. **The pick is also what we advertise to Copilot**: since Copilot derives the prompt budget as (context − max output), **picking a shorter answer cap hands the freed tokens back to your prompt**. A **shorter** pick lands on the very next response's `max_tokens` instantly; a **longer** pick lands once the extension re-publishes model metadata on the first request after the change — the same one-request lag mode switches have, because Copilot sizes your prompt against the advertised output and the wire never promises more than Copilot was told. The menu itself stays anchored to the model's ceiling, so picking 16K never removes 32K from the list. **All 14 bundled presets now ship an Output length ladder**: the seven 1M-context models (DeepSeek V4 Pro/Flash, GLM-5.2, GLM-5.3, GLM-5.3-Flash, Kimi K3, MiniMax M3) top out at **128K** (Kimi's own API default; GLM card benchmarks generate up to 164K), the 262K-class models run 32K down to **8K** for quick answers, and Laguna-S-2.1's default rose to 64K because its thinking turns routinely blow past 32K. Pick a shorter length on small serve windows to hand tokens back to the prompt. VS Code remembers your choice per model — nothing new is written to your settings.
- **Model-picker banners** — when a model's output budget is clamped below what you configured (by the context window or a provider cap), the picker entry now shows a warning / info line explaining why, so the limit is visible right where you pick the model instead of buried in the dashboard. The `infoText` variant is runtime-gated to VS Code ≥ 1.135.

### Changed

- **Minimum VS Code raised to 1.128** — the `configurationSchema` / `modelConfiguration` picker APIs that the output-length dropdown and banners ride on require it.
