...
- **OpenRouter dashboard shows only correct-scope data (interim)** — until the model-collection restructure (Phase 2), OpenRouter relay nodes **suppress the Models node and Context Window row**: `/v1/models` is OpenRouter's whole catalog (not "the server's models"), and a single configured model's context is not the server's. Per-model detail (context, pricing, capabilities) lands with Phase 2.
