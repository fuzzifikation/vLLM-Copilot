...
3. **`sessions.cwd` is a folder path, not a workspace-storage id** (`g:\vLLM-Copilot`), so the picker resolves `workspace.json` → folder → `cwd`. On Windows, both sides fold separators and case through `normalizeCwd()`; POSIX preserves backslashes and case so distinct folders cannot collide. A **multi-root** workspace checks every root, matches each resolvable root, and counts equivalent normalized paths only once. An unresolvable root is never guessed into a catalog `cwd` and makes scoped cleanup incomplete. Remote workspaces (`vscode-remote://…`) have no local path to reconstruct, so they stay listed for their on-disk residue but are never matched against `cwd`.
...
The catalog-only, global and whole-machine picker entries are deliberately distinct:

| Entry | Scope |
|---|---|
| **All global sessions** | the global `state.vscdb` index, `emptyWindowChatSessions/`, and catalog rows with `cwd IS NULL` |
| **Unattributed catalog sessions** | rows with a non-null `cwd` that matches no readable workspace folder; explicitly selected catalog rows only, with no workspace files or repo memory inferred |
| **EVERY conversation on this machine** | the whole catalog — every session in every project, plus orphaned child rows |
| **Maintain Copilot catalog** | rebuild the search index, VACUUM, and truncate the WAL; no conversations, orphaned rows or workspace files deleted |

If only unattributed rows remain, the picker still opens so this scoped choice and the whole-catalog option are available. If child rows have lost their session parent, their workspace is unknowable: scoped cleanup leaves them and warns, while the confirmed whole-catalog option stays available even when only orphaned rows remain. Maintenance never removes those rows; it remains available to retry a failed compaction when no sessions remain, and must be selected on its own. Memory options also remain available when only repo or global user memory is left. A selected workspace with even one unresolvable root is warned before deletion and reported incomplete afterwards; its resolvable roots can still be cleaned, but no catalog rows are guessed for the others.

### Failure behaviour

Errors reading the session index or catalog, or listing session directories (other than absent paths), stop discovery before any deletion choice is offered. Unreadable workspace folder metadata instead leaves only known folders attributable; a selected workspace is warned before cleanup and reported incomplete afterwards. The catalog delete runs before local file and memory removal, in a single `BEGIN IMMEDIATE` transaction covering the parent, every child table and the full-text index rebuild, driven by ONE `where` clause so the two delete statements cannot disagree about scope. An unknown session-keyed table with rows for selected sessions blocks the operation before deleting any parents, children, workspace files or memory. Unknown rows outside a scoped selection do not block it; a whole-catalog wipe blocks on any unknown session-keyed rows. If a known table has been renamed or dropped, or rebuilding fails, the transaction rolls back and reports `agentStoreError`, leaving workspace data and catalog conversations discoverable for retry. Directory removal failures are reported separately from absent directories; memory-only and partial memory removals are reported from actual results.

Afterwards the file is `VACUUM`ed and then `PRAGMA wal_checkpoint(TRUNCATE)`d. Both are required and neither implies the other: this database runs in WAL mode, and `VACUUM` only rewrites the MAIN file, so the pages it frees are written to the `-wal` sidecar. A reproduction of this exact sequence left a 4 KB clean main file beside a 2.1 MB sidecar still containing the deleted conversation text, which is why `compacted` used to describe a file that was not the file holding the data. `TRUNCATE` is the checkpoint variant that also empties the sidecar; `busy` non-zero means a concurrent reader blocked it. If either step fails after the delete commits, the summary reports the removed rows separately from physical cleanup. Closing VS Code alone does not rerun VACUUM: reopen **Clean Copilot Sessions** and select **Maintain Copilot catalog** after other readers release the file. Maintenance rebuilds the FTS5 index and retries VACUUM/checkpoint without deleting conversations; it reports index rebuilding and compaction separately.

None of that is sufficient on its own. `search_index` is an fts5 table, and deleting a row from an fts5 table does not delete its postings: the index keeps them as tombstones until the segments are merged or the index is rebuilt, and `VACUUM` copies those live pages verbatim. Measured on this sequence, the deleted conversation's indexed term was still present in the MAIN file after the row delete, the `VACUUM` and the truncating checkpoint, while the table correctly returned zero rows for it. So the child delete is followed by `INSERT INTO search_index(search_index) VALUES('rebuild')`, which re-reads the table's remaining content and discards every posting belonging to a deleted row. Rebuilding runs before `COMMIT`; if it fails, every catalog delete rolls back and the sessions remain discoverable for a retry. VACUUM/checkpoint run only after a successful commit, and their failures are reported separately.
