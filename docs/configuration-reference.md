...
|-------|:-------:|-------------|
| `id` | — | **Required.** Unique key referenced by each model's `server`. The Add flow generates a slug from the host/port (e.g. `localhost-8000`; colliding slugs get `-2`, `-3`, …). Ids are stable write targets — identity stays the URL+headers fingerprint. |
