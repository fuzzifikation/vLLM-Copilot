...
The price sits on the **model line** (its collapsed summary: `$11.51 today and $31.13 total`); the **Today / Overall** rows are token-only - `800k in · 200k cached · 500k out`, where `in` **excludes** cache and `in + cached = total input` (cached = cache-*read* input tokens). Costs use **fine precision** - sub-cent amounts keep up to 6 decimals (`$0.0007`) rather than collapsing to `$0.00`. **OpenRouter models prefer their actual reported cost (`usage.cost`) when present**, falling back to the configured per-1M rates; the two are never summed. Token counts round to whole thousands. **There is no server-level cost sum** - each model carries its own rate, and summing them is the user's call. Currency is USD only; an unrecognised hand-edited `currency` falls back to its raw label so a non-dollar amount never renders as a wrong `$`; no currency library is bundled.
...
- **`currency`** is a display label only - USD is the only unit used, and the Set Cost command writes it automatically. It exists so a hand-edited settings.json shows an unrecognised unit verbatim rather than a wrong `$`.
