...
| 18 | Copilot session janitor | F | `commands/cleanSessions` (UI) + `shared/sessionManager` (engine) |
...
flowchart TD
    ...
    CAT --> PARSE[parseOpenRouterCatalogData - module-private, one parse per fetch]
...
flowchart TD
    ...
    DISC --> FOLD[resolveWorkspaceFolders: folder / folders / workspace pointer, fileURLToPath]
...
- Executed unit 2 (dead-name hygiene, gauntlet-green + independent audit: five EQUIVALENT verdicts, three review nitpocks fixed same pass): PF-1 `parseOpenRouterCatalogData` un-exported, phantom generic dropped, docstring lie replaced (census confirmation: it was already `INTERNAL_SINGLE` at baseline - the "shared with the metrics engine" export never had one external customer); PB-3 `parseToolCallArgs.onUnparseable` parameter deleted (zero production callers; the deleted `onUnparseable?.()` was a no-op on `undefined`); P A-1 `planOutputLengthMigration` + `OutputLengthProposal` module-private, `test/outputLengthMigration.test.ts` rebuilt to drive `maybeOfferOutputLengthMigration` end to end - skip paths now assert silence triple (no toast, no write, no flag) and the `''` CLEAR proposals now pin the WRITTEN entry (`modelModes`/`defaultParams` absent via `CLEARABLE_ON_EMPTY`), strictly more than the old proposal-payload stops; PF-3 the `readWorkspaceFolders` wrapper (zero prod callers, discarded `unresolved`) deleted, `resolveWorkspaceFolders` exported, five rerouted cases gained flag assertions incl. remote `unresolved=true`; PF-4 `getBundledPresetNameToBasename` inlined into its single caller, anti-hijack comment moved with it. Census: `readWorkspaceFolders`/`getBundledPresetNameToBasename` gone, `planOutputLengthMigration` TEST_ONLY → `INTERNAL_SINGLE`, `resolveWorkspaceFolders` exported as the honest test seam (`TEST_ONLY` by design, PF-3 ruling).
- Executed unit 3 (relocations, gauntlet-green + independent audit: every moved block byte-identical to HEAD, six items EQUIVALENT, zero drift): P C-1 the whole janitor UI (banner, four id constants, `SessionPick`, `registerCleanSessionsCommand`, `isMemoryOption`, `confirmClean`, `reportClean`) carved out of `commands.ts` into `commands/cleanSessions.ts` - the file is now 664 lines (was 1,127) and holds only server-registry CRUD + usage/cost registrations; the header's "stable public facade, re-exports workflows" claim was a provable lie (HEAD had zero re-export statements) and the new header states the truth: NOT a facade, domain-owned commands are imported by `extension.ts` from their files. P C-2 (precedent R7-P5-1/2): `registerDiagnoseConnectionCommand` to `ui/diagnostics.ts` (gained one `state/config` import, ui->state lane pre-existing via `dashboard.ts`), `registerOpenLogFileCommand`/`registerClearLogFilesCommand` to `shared/logger.ts` (zero new imports, `FileLogger` same-file), `registerOpenDashboardWebCommand` to `ui/dashboard.ts` (zero new imports). Census: exactly the 8 moved functions + 1 type + 4 constants relocated with unchanged rent classes; `registerCleanSessionsCommand` stays `BIG_SINGLE` ENTRY wiring (extension activation); cluster modularity rose 0.792 -> 0.803 with the new module. Review-tooling hygiene found by the audit: `.tools/` (the review agent's sandbox) was not gitignored - now it is.
...
owner's choice. All three units executed 2026-09-26, one commit each,
census-diffed and independently audited (every published claim byte-checked,
reviewer corrections recorded in the unit bullets above).
