...
- **Remote setups (SSH/WSL/Containers)** - the extension must run on the remote workspace host (`extensionKind: workspace`) because requests originate beside the remote server. Install it while connected to the remote window; a local-first install won't be picked up by the remote automatically. The local UI host remains the home of VS Code client data, which a future config-file backend can access through VS Code's local file service. See the note in [Quick Start](../README.md#quick-start).
...
- **Chat session cleanup.** The **Clean Copilot Sessions** command wipes selected workspaces' history, including conversation text and its full-text index inside Copilot's session catalog, then attempts to compact the database. An incomplete scan blocks deletion; failed directory removals are reported. Unknown catalog tables holding selected session data block deletion before workspace files are touched. Unmatched catalog paths have a separate catalog-only picker entry; orphaned catalog text cannot be assigned to a workspace and requires the explicitly confirmed whole-catalog wipe, which remains available even without live sessions. **Maintain Copilot catalog** rebuilds the index and retries compaction without deleting conversations, but does not remove orphaned text. Two optional checkboxes delete Copilot's memory files even after sessions are gone; the global one affects every workspace on the machine. See [copilot-integration.md](copilot-integration.md) for the storage layout it acts on.
