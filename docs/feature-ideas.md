...
**Updated:** 2026-09-03 (completed items removed; architecture section added for maintainer-side structural ideas with build triggers)
...
> **Tracking:** Only two docs are maintained: this file (new ideas) and [code-review.md](./code-review.md) (real bugs and nice-to-have refactors against the current code). There is no consolidated roadmap.
...
**Cross-note:** This *reconsiders* the per-model identity decision in [code-review.md](./code-review.md) ("Accepted product decisions: Server registry"). It is **not** a return to the deprecated single-global-server; it's an explicit registry of *N* named servers that models reference.
...
**Build trigger:** the next time a config migration is needed. Write it ON the rails, in the same commit that extracts them.
