...
- **Triple declaration of the context-resolver signature** — `resolveContextWindow` (standalone) + `getModelContextWindow` (1-line passthrough method) + `contracts.ts` interface. The seam is defensible; the passthrough exists only to satisfy the `Pick` in discovery. Not worth a refactor for its own sake. Do not re-file.
