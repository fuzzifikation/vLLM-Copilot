...
- **`findModelCost` takes the first match when two presets on one server share a wire id but have different `cost` configs** — `usageStore.findModelCost` (and the per-model Token Usage cost row, and the Last Request Cost row) resolve by `(normalized serverUrl, wire modelId)` and return the first entry's `cost`. Two presets on one server that both resolve to the same wire id but carry different per-1M rates would use whichever appears first in the `models` array. Fringe because: sharing a wire id across presets on one server is the multi-alias edge case, and the effect is a cost label the user can visibly cross-check against the model's configured `cost`. Fixing would need a disambiguation rule (first-match is already the documented behavior of `findModelConfigIndex`'s identity model).
...
- **Triple declaration of the context-resolver signature** — `resolveRuntimeLimits` (standalone) + `getModelContextWindow` (1-line passthrough method) + `contracts.ts` interface. The seam is defensible; the passthrough exists only to satisfy the `Pick` in discovery. Not worth a refactor for its own sake. Do not re-file. (As of the OpenRouter Step 1 prep, the standalone and the method return `RuntimeModelLimits` instead of a bare number — the widening itself is deliberate per openrouter-plan.md; this note covers only the *triple-declaration* seam, which remains accepted.)
- **`requestBuilder` re-derives server config** — `resolveServerConfig` returns `{ serverUrl, requestHeaders }`, then `buildRequest` rebuilds it into a `ServerConfig` by adding two fields. Could be one resolver returning `ServerConfig` directly. Minor cohesion nit; not worth it. Do not re-file.
...
## Code Review 2026-08-12 — deferred architecture (agreed, not yet tackled)

Agreed with the user to write these down and tackle later. They are real maintainability concerns, not bugs. Fixes 1+2 (non-vLLM auto-continue duplication; no timeout on the initial chat POST) shipped in this pass — see CHANGELOG.

### Deferred (must tackle, but later)

- **#9 — Request construction split across `provider/requestBuilder.ts` and `vllmClient.ts`** — `buildRequest` (provider/requestBuilder.ts) builds the skeleton, then `buildChatBody`/`validateMessages` (vllmClient.ts) mutate and finish it. The body-construction logic lives in the transport/client module, so the "client" isn't purely transport. **Plan: move body construction fully into requestBuilder.ts; shrink `VllmClient` to transport + config cache.**
- **#10 — `dashboard.ts` still excluded from coverage despite carrying features** — the dashboard gained degraded notices and measured throughput, but remains the sole excluded-from-coverage source file. Feature-bearing code without coverage is how the earlier dashboard regressions happened. **Plan: reinstate coverage and close the gaps.**
- **#11 — `VllmClient` is a god-object** — config cache owner + context-window resolver + server-type detection + streaming + message validation + logging all in one class. #9 is the first peel; the remaining axes (resolver, streaming, validation) can each move out. **Plan: split after #9 lands.**
