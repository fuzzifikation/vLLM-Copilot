...
- **Server registry** — servers are entries in the global `vllm-copilot.servers` registry (`serverUrl`/`requestHeaders`/`serverType` live there); models only reference an entry via `server`. There is no global/default server: nothing resolves an entry without a model reference. The URL+sanitized-headers fingerprint remains the identity unit for grouping.
