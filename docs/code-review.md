...
- **SESSION-1, priority two: re-audit Clean Copilot Sessions against VS Code 1.139 session storage.** VS Code 1.139 describes a central Agent Host session catalog backed by individual session and chat databases, while `src/shared/sessionManager.ts` still deletes ten hardcoded `state.vscdb` keys and older session directories. There is no public extension API for this cleanup. Establish whether the command still removes current sessions, map the live storage topology on 1.134 through 1.139, and prefer supported session UI or commands where possible. Remove the command instead of extending private storage surgery if the new foundation makes honest support impractical.
