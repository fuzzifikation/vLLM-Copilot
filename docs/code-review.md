...
- **SESSION-1 (deferred 2026-09-25): re-audit Clean Copilot Sessions against VS Code 1.139 session storage.** VS Code 1.139 describes a central Agent Host session catalog backed by individual session and chat databases, while `src/shared/sessionManager.ts` still deletes ten hardcoded `state.vscdb` keys and older session directories. There is no public extension API for this cleanup. Owner observation: session cleaning still appears to work in practice. That is not the same as verified — the command has not been checked against the 1.139 topology, so a moved key set could still silently no-op for newer sessions. Deferred until the owner has determined what the 1.139 session storage actually is. When that happens: map the live storage topology on 1.134 through 1.139, verify the command removes current sessions (not just that it runs without error), and prefer supported session UI or commands. Do not extend the private storage surgery to cover new keys — if the old foundation is gone, remove the command instead.
- **AGENT-1: verify the first-model Agent Host bridge in a live window.** The bootstrap now runs from the central model-setting change hook and refuses to opt in when the effective model array is empty, but no dev-host test has proved that a first model added after activation becomes visible in an already running Agents window. Verify local Chat, Remote-SSH or WSL, an already open Agents window, first-time model addition, explicit opt-out preservation, and the config-schema tool's availability. Keep this live until the end-to-end matrix passes.
- **VSCODE-1: decide whether to raise the minimum VS Code engine to 1.134.** The manifest currently requires `^1.128.0`; the lockfile already resolves `@types/vscode` 1.134.0, while current Stable is 1.139.0. A 1.134 floor gives newer stable declarations without adopting a two-day-old engine requirement. Before changing it, identify every runtime capability that still needs a gate below 1.134 and run the compatibility matrix against 1.134 rather than only current Stable. (Owner ruling 2026-09-25: deferred — no API newer than 1.128 is known to be needed. Re-evaluate when `docs/feature-ideas.md` proposes work that depends on a newer API; raising the floor without a consumer is churn.)
...
- **`style-src 'unsafe-inline'` in the webview CSP** (ratified 2026-09-25). All three shipped webviews set element styles directly at runtime, so the exception is deliberate. `script-src` stays locked to `${cspSource}` with no `unsafe-inline` or `unsafe-eval`, which is the part that actually matters. `.github/copilot-instructions.md` now documents the exception; do not re-file this and do not refactor inline styles into CSS files to chase it.
- **Configured `contextWindow` must be a whole number above 50,000** (`isValidContextWindow`). Intentional floor, ratified 2026-09-10 after an external review filed it as "rejects valid 32K gateways": below ~50k Copilot has no usable prompt+output headroom, so the manual fallback refuses to relocate the failure. The asymmetry with server-reported windows (any positive value) is the point - the server's number is authoritative, a typed guess is not. Do not re-file.
