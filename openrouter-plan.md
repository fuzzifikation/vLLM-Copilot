...
**Status (2026-08-21):** Onboarding, cost data plane, model-collection dashboard, provider pinning, routing modes (Standard/Nitro/Exacto), and per-provider limits + pricing are all SHIPPED **and pushed** (`main` @ `823c974`, clean tree). The **responsibility/display decision** for per-provider limits is LANDED — the extension displays live and never persists, and the user owns the provider choice. Phase 3 (activity ledger) stays **DEFERRED** (undocumented endpoint + management-key scope). The plan's core goals are complete; this document is the running record.

**Where we are now → COMPLETE (2026-08-21):** per-provider limits (`context_length` / `max_completion_tokens`) are surfaced live and read-only — the Model Settings Provider dropdown shows each provider's context + output + per-1M pricing, the dashboard shows the pinned provider's own limits, and a symmetric Attention icon flags any output budget clamped below the configured value (catalog ceiling or pinned provider cap). **Never persisted, never clamped, never special-cased** (the old "clamp at pin time" idea and the per-error enrichment are rejected — the user owns the provider choice, and the generic OpenRouter error path surfaces code + formatted message for all failures, `constraint_filtered` included). No further item remains (see [implementation checklist](#next-step-implementation-checklist)).
...
### Shipped in this delivery (2026-08-21) — all items DONE

**Per-provider limits: lazy display, no persistence, user-owned choice.** Per-provider `context_length` / `max_completion_tokens` (from `/api/v1/models/{id}/endpoints`) are surfaced **live and read-only** — in the pin dropdown, on the dashboard, and (for failures) through the generic OpenRouter error path — **never persisted to settings**. See [Provider-Level Token Evaluation](#provider-level-token-evaluation-2026-08-20-verified-live-against-the-api) and [Per-Provider Limits — Responsibility & Display Decision](#per-provider-limits--responsibility--display-decision-2026-08-21). The 80/90 use case (Auto routing) is already safe and needs nothing.

**Output-budget guard: clamp + dashboard icon.** The general/catalog output ceiling clamps the user's `maxOutputTokens` silently in `deriveTokenBudget()` — and Copilot already advertises the clamped value. The dashboard Attention icon flags any binding constraint (catalog ceiling or pinned provider cap) that pushes the effective output below the configured budget. **No popup, no session tracking, no selection tracking** (see [Output-budget guard](#output-budget-guard-clamp--dashboard-icon-symmetric-across-all-constraints)). When the user changes settings in Model Settings, Copilot is refreshed automatically via the existing `onDidChangeConfiguration → clearCache() → re-discovery` chain — the clamped value is recomputed at every discovery.
