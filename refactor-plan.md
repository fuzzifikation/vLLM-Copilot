...
| 3d | ✅ Apply the same undefined-stripping invariant to `replaceModelConfig` (both merge and append paths) | each new test red before its implementation, then green; full suite green — `stripUndefined` hoisted to the top of `replaceModelConfig`; 2 new tests in `test/configStore.test.ts` (red on merge + append paths pre-fix); 467 suite green |
...
Do not combine 3a and 3b: replace/patch is the highest-risk boundary and each migration must be independently reviewable. Step 3c contains intentional hardening and must not be described as a mechanical move. Step 3d closes the replace-path counterpart of the 3c hardening. Later extraction steps remain behavior-preserving.
...
8. **Patch-mode `undefined` spread hazard (theoretical)** — `serverSettingsView.ts:336` `{...existingEntry, ...updates}`: an explicit `serverUrl: undefined` in `updates` would overwrite the stored value. JSON messages never carry `undefined`, so the webview is safe today; the unified store should strip `undefined`-valued keys in patch mode (hardening, not a bug). *Resolved in step 3c — `stripUndefined` in `patchModelConfig`, plus a runtime backstop so `id`/`serverUrl` cannot be moved via updates.* The replace path had the same leak — `{...entry}` spread persisted undefined-valued keys in replace mode — closed in step 3d (`stripUndefined` hoisted to the top of `replaceModelConfig`, both merge and append paths; note replace does NOT preserve the wiped field the way patch does — the invariant is that top-level undefined is never written, not that the previous value survives). Scope is top-level only: `stripUndefined` does not recurse into nested objects (`defaultParams`/`modelModes`/`capabilities`), and nested undefined is deliberately left as-is — it is inert, since reads and JSON serialization treat absent and undefined identically (pinned by test).
...
| Patch-mode `undefined` spread hazard | §4.2 step 3c | Strip `undefined` update keys (hardening) — *done 3c*; replace-path counterpart (never persist top-level `undefined`, merge + append) — *done 3d* |
