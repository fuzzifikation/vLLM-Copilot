...
- \`contextWindow\` — hidden fallback used only when the requested model exists in /v1/models but its row omits a positive \`max_model_len\`. A server-reported value always wins. Whole number **above 50,000** tokens — below that Copilot has no usable headroom.\n- \`personality\` — name of a SHIPPED preset (e.g. \"Sarcastic Robot\"). Portable across machines/OSes (each install seeds its own copy); \`systemMessageReplacementsFile\` is for user-defined files only. Name outranks a stale path when both are set.\n- Sampling params are model-specific. When unsure, start from the model's HF card; common safe defaults: \`temperature: 0.7\`, \`top_p: 0.95\` for coding; \`temperature: 1.0\` for general reasoning.
