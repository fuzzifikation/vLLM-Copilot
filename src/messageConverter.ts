 ...
 *
 * Handles all three VS Code message roles (dispatch branches inline — the
 * two former per-role functions had exactly one caller each):
 * - `System` → passed through as `role: 'system'` (OpenAI supports this; Copilot
 *   may inject system messages for agent instructions).
 * - `Assistant` → text + reasoning + tool calls; dropped when all are empty.
...
export function convertMessages(
  ...
  for (const msg of messages) {
    if (msg.role === vscode.LanguageModelChatMessageRole.Assistant) {
      ...
      // User: text/image parts + tool results (tool results first — they
      // respond to the previous assistant's tool_calls).
      ...
      for (const part of msg.content) {
        if (part instanceof vscode.LanguageModelTextPart) {
          ...
          const imagePart = part as vscode.LanguageModelDataPart;
          const base64 = Buffer.from(imagePart.data).toString('base64');
          contentParts.push({
            type: 'image_url',
            image_url: { url: `data:${imagePart.mimeType};base64,${base64}` },
          });
        }
      }
      ...
      if (contentParts.length === 0 && toolResults.length === 0) {
        otherMessages.push({ role: 'user', content: '' });
      }
    } else {
...
export function formatError(err: unknown): string {
  ...
  for (const text of [msg, ...allCauses]) {
    ...
    // The server's own message, in order of honesty:
    // 1. OpenAI-compatible `{"error":{"message":"…"}}` body. OpenRouter buries
    //    the real provider reason under `error.metadata.raw` while
    //    `error.message` is a terse stub — so every message-like field is
    //    gathered and joined, not dead-ending on the stub. Full JSON parse
    //    first (correct escaping); a tolerant regex recovers `message` + `raw`
    //    even when fetchWithRetry truncated the embedded body.
    ...
    return `Server error [${m[1]}]${message ? `. ${message}` : '.'}`;
  }
