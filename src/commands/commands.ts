/**
 * Thin per-command registrations: server registry CRUD (auth, rename, move,
 * remove server/model), usage reset, and cost configuration. Each function
 * registers one `vllm-copilot.*` command and funnels settings writes through
 * `attemptWrite` so a rejected write never surfaces as the anonymous
 * "command failed" toast (CR-48).
 *
 * NOT a facade and no re-exports: every domain-owned command is imported by
 * `extension.ts` straight from its file (addServerFlow, autoConfigureFlow,
 * byok, testAndRefresh, personality, cleanSessions from `src/commands`;
 * diagnose from `ui/diagnostics`, deep-dive/selector/poll-interval from
 * `ui/*`, the log-file commands from `shared/logger`). The header used to
 * claim the opposite; nothing ever imported this file that way.
 */
